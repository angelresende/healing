...
from paciente.models import Consulta, Documento_new
...
def cadastro_medico(request):
  ...
  if request.method == "GET":
    especialidades = Especialidades.objects.all().order_by('especialidade')
...
def consulta_area_medico(request, id_consulta):
  ...
  if request.method == "GET":
    ...
    documentos = Documento_new.objects.filter(consulta=consulta)
    ...
    if request.user != consulta.data_aberta.user:
      messages.add_message(request, constants.ERROR, 'A consulta que você está tentando finalizar não é sua!')
